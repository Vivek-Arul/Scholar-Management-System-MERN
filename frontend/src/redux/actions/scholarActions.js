...
export const addScholar = (scholar) => async (dispatch) => {
  try {
    const response = await axios.post('http://localhost:5000/api/scholars', scholar);
...
export const updateScholar = (scholar) => async (dispatch) => {
  try {
    const response = await axios.put(`http://localhost:5000/api/scholars/${scholar.orcid}`, scholar);
...
export const deleteScholar = (orcid) => async (dispatch) => {
  try {
    await axios.delete(`http://localhost:5000/api/scholars/${orcid}`);
